...
export class ObjectHandler {
    ...
    constructor(
        ...
        this.tabsStorage.addTab({
            id: 'tab-1',
            status: 'free',
            type: this.showWelcomePage() ? 'welcome' : 'cwc',
            isSelected: true,
        })
        ...
        this.connector = new Connector({
            ...
            onCWCContextCommandMessage: (message: ChatItem, command?: string): string | undefined => {
                ...
                if (command === 'aws.amazonq.sendToPrompt') {
                    ...
                    if (tabID && command) {
                        postMessage(createOpenAgentTelemetry('cwc', 'right-click'))
                        postMessage({
     ...
     ${message}`,
        ...
        this.mynahUIProps = {
            onReady: () => {
                // the legacy event flow adds event listeners to the window, we want to avoid those in the lsp flow, since
                // those are handled by the flare chat-client
                if (this.mynahUI === undefined && this.connector) {
            ...
            onChatPrompt: (tabID: string, prompt: ChatPrompt, eventId: string | undefined) => {
                ...
                if (prompt.command !== undefined && prompt.command.trim() !== '') {
                    ...
                    if (newTabType) {
                        postMessage(createOpenAgentTelemetry(newTabType, 'quick-action'))
                    }
                    return
                }
            ...
            onInBodyButtonClicked: (tabId, messageId, action, eventId) => {
                switch (action.id) {
                    case disclaimerAcknowledgeButtonId: {
                        ...
                        // create telemetry
                        postMessage(createClickTelemetry('amazonq-disclaimer-acknowledge-button'))
                    ...
                    case 'quick-start': {
                        ...
                        postMessage(createClickTelemetry('amazonq-welcome-quick-start-button'))
                        return
                    }
                    case 'explore': {
                        ...
                        postMessage(createClickTelemetry('amazonq-welcome-explore-button'))
                        return
                    }
                    default: {
        ...
        if (mynahUI) {
            this.mynahUI = new MynahUI(this.mynahUIProps)
        }
    ...
    private showWelcomePage() {
        return this.welcomeCount < welcomeCountThreshold
    }
